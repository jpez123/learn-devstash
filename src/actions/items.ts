...
export async function toggleFavoriteItem(itemId: string): Promise<ActionResult<{ isFavorite: boolean }>> {
  const session = await auth();
  if (!session?.user?.id) {
    return { success: false, error: 'Unauthorized' };
  }
  ...
  try {
    const isFavorite = await toggleFavoriteItemDb(itemId, session.user.id);
    if (isFavorite === null) {
      return { success: false, error: 'Item not found or access denied' };
    }
